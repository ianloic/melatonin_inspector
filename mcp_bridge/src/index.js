...
class IpcClient {
                ...
                try {
                    ...
                    if (pending) {
                        if (json.status === 'ok')
                            ...
                            pending.reject(new Error(json.data));
    ...
    async send(action, args = {}) {
        return new Promise((resolve, reject) => {
            if (!this.isConnected)
                return reject(new Error("Not connected to JUCE application. Ensure the app is running and Inspector is open."));
            ...
            try {
                switch (request.params.name) {
                    ...
                    case "juce_set_component_bounds": {
                        const { id, x, y, w, h } = request.params.arguments;
