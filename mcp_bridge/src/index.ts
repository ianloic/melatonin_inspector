...
class IpcClient {
    ...
    async send(action: string, args: any = {}): Promise<any> {
        return new Promise((resolve, reject) => {
            if (!this.isConnected) return reject(new Error("Not connected to JUCE application. Ensure the app is running and Inspector is open."));
            ...
            tools: [
                {
                    ...
                    description: "Retrieve the full hierarchical tree of JUCE Components from the currently running app.",
                    inputSchema: { type: "object", properties: {} }
                },
                {
                    name: "juce_get_component_details",
                    description: "Get detailed properties of a specific JUCE Component (bounds, colors, fonts, etc.).",
                    inputSchema: {
                        type: "object",
                        properties: {
                            id: { type: "string", description: "The component ID (pointer string) returned from get_tree" }
                        },
                        required: ["id"]
                    }
                },
                {
                    ...
                    description: "Visually highlight a component on the screen for debugging.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            id: { type: "string" }
                ...
                },
                {
                    ...
                    description: "Sends a request to quit the running JUCE application gracefully.",
                    inputSchema: { type: "object", properties: {} }
                }
            ]
        }));
            ...
            try {
                switch (request.params.name) {
                    ...
                    case "juce_highlight_component": {
                        ...
                        return { content: [{ type: "text", text: `Component ${id} highlighted.` }] };
                    }
                    case "juce_click_component": {
                        const id = String(request.params.arguments?.id);
                        await this.ipcClient.send("click", { id });
                        return { content: [{ type: "text", text: `Click dispatched to component ${id}.` }] };
                    }
                    case "juce_set_component_bounds": {
                        ...
                        return { content: [{ type: "text", text: `Bounds updated for component ${id}.` }] };
                    }
